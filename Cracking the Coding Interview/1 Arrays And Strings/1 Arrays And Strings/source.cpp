...
void getPerms(string permuted, string unpermuted, vector<string> &perms)
{
	if (unpermuted.length() == 1)
	{
		perms.push_back(permuted + unpermuted);
	}
	else
	{
		for (unsigned int i = 0; i < unpermuted.length(); ++i)
		{
			string newPermuted = permuted + unpermuted[i];
			string newUnpermuted = unpermuted.substr(0, i);

			if (i + 1 < unpermuted.length())
				newUnpermuted = unpermuted.substr(0, i) + unpermuted.substr(i + 1);

			getPerms(newPermuted, newUnpermuted, perms);
		}
	}
}
...
void buildPermTree(Node* root, string str)
{
	for (unsigned int i = 0; i < str.length(); ++i)
	{
		...
		if (i + 1 < str.length())
			temp += str.substr(i + 1);
		buildPermTree(node, temp);
	}
}

void getPermsDFS(Node* root, vector<string> &perms, string curStr)
{
	if (root->children->empty())
	{
		perms.push_back(curStr);
		return;
	}

	for (auto it = root->children->begin(); it != root->children->end(); ++it)
	{
		string temp = curStr + (*it)->value;
		getPermsDFS(*it, perms, temp);
	}
}
...
vector<string> getPermutationsTree(string str)
{
	vector<string> perms;
	if (str.length() != 0)
	{
		...
		buildPermTree(root, str);
		getPermsDFS(root, perms, "");
		deleteTree(root);
	}
...
int main()
{
	...
	string perm = "abcd";
	int count = 0;
	//vector<string> perms = getPermutations(perm);
	vector<string> perms = getPermutationsTree(perm);
	cout << "Permutations of " << perm << " are:" << endl;
